...
```{r make-state-map, echo=FALSE, fig.width = 10, fig.show="animate"}
...
```{r make-county-map, echo=FALSE, fig.show="animate", cache.lazy=FALSE}
# specify dates for which historic map data is to be gathered
...
```{r ref.label="make-state-map", echo=TRUE, eval=FALSE}
```

### County-level map
```{r ref.label="make-county-map", echo=TRUE, eval=FALSE}
```

## Final notes
The Rmd file used to create this report can be downloaded [here](https://github.com/kweatherholtz/kweatherholtz.github.io/blob/master/_R/2016-07-05-animated-map-of-US-settlement.Rmd).
